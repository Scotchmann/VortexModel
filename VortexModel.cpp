...
int DrawChart(CumulativeVector * _CumuCon)
{
    ...
    {
		if( ((CumulativeVector)(*_CumuCon))[i]->getValue() > 0
		&&	((CumulativeVector)(*_CumuCon))[i]->getReliability() > 0)
		{
            // формируем уровни треугольной матрицы на графике
            arrY.push_back(((CumulativeVector)(*_CumuCon))[i]->getDistance() );
			// формируем уровни надежности на графике
            arrX.push_back(((CumulativeVector)(*_CumuCon))[i]->getReliability()  );
...
{
    return ptr_UpVortex->getAgent(i, j);
}

int GetDistance()
{
    return ptr_UpVortex->getDistance();
}

double GetBuf()
{
    return ptr_UpVortex->getBuf();
}
